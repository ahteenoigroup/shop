...
const ADMIN_ENTITIES = Object.freeze({
  ...
  customization_groups: {
    sheet: "Customization_Groups",
    id: "group_id",
    prefix: "GRP",
    width: 3,
  },
  customization_options: {
    sheet: "Customization_Options",
    id: "option_id",
    prefix: "OPT",
    width: 3,
  },
...
function runSetupAdmin() {
  const result = setupAdmin("P@ssw0rd");
  console.log(result);
}
    ...
    switch (action) {
      case "create_customer":
        data = withScriptLock_(function () {
          return createCustomer_(body);
        });
        break;
      case "save_address":
        data = withScriptLock_(function () {
          return saveAddress_(body);
        });
        break;
      case "create_order":
        data = withScriptLock_(function () {
      ...
      case "update_payment_status":
        data = withScriptLock_(function () {
          return updatePaymentStatus_(body);
        });
        break;
      case "admin_auth":
      ...
      case "admin_upsert":
        assertAdmin_(body.admin_key);
        data = withScriptLock_(function () {
          return adminUpsert_(body);
        });
        break;
      case "admin_delete":
        assertAdmin_(body.admin_key);
        data = withScriptLock_(function () {
          return adminDelete_(body);
        });
        break;
      case "admin_update_order":
        assertAdmin_(body.admin_key);
        data = withScriptLock_(function () {
          return adminUpdateOrder_(body);
        });
        break;
      default:
...
function assertAdmin_(providedKey) {
  ...
  if (!expectedKey) {
    throw apiError_(
      ...
      "ยังไม่ได้ตั้งค่า Admin key กรุณารัน setupAdmin('รหัสของคุณ')",
      503,
    );
  }
...
function getAdminSnapshot_() {
  ...
  return {
    stats: {
      ...
      active_restaurants: restaurants.filter(function (row) {
        return toBoolean_(row.is_active);
      }).length,
      menu_items: menuItems.length,
      available_menu_items: menuItems.filter(function (row) {
        return toBoolean_(row.is_available);
      }).length,
      orders: orders.length,
      active_orders: orders.filter(function (row) {
        return (
          ["received", "preparing", "delivering"].indexOf(
      ...
      today_revenue: roundMoney_(
        todayOrders.reduce(function (sum, row) {
          return (
            sum +
            (String(row.payment_status) === "paid" ? Number(row.total || 0) : 0)
          );
        }, 0),
      ),
      total_revenue: roundMoney_(
        orders.reduce(function (sum, row) {
          return (
            sum +
            (String(row.payment_status) === "paid" ? Number(row.total || 0) : 0)
          );
...
function getAdminEntity_(entity) {
  ...
  if (!config)
    throw apiError_("UNKNOWN_ENTITY", "ไม่รู้จัก entity: " + entity, 400);
  return config;
}
...
function adminList_(body) {
  ...
  return {
    entity: body.entity,
    headers: getHeaders_(getSheet_(config.sheet)),
    rows: rows,
  };
}
...
function adminUpsert_(body) {
  ...
  clean[config.id] = id;
  if (headers.indexOf("created_at") !== -1 && !clean.created_at)
    clean.created_at = timestamp;
  if (headers.indexOf("updated_at") !== -1 && !clean.updated_at)
...
function adminDelete_(body) {
  ...
  if (
    ["orders", "order_items", "payments", "order_status_history"].indexOf(
      String(body.entity),
    ) !== -1
  ) {
...
function adminUpdateOrder_(body) {
  ...
  if (Object.prototype.hasOwnProperty.call(body, "rider_id"))
    changes.rider_id = body.rider_id;
...
function listRestaurants_(params) {
  ...
  const categoryById = indexBy_(
    readTable_(CONFIG.SHEETS.CATEGORIES),
    "category_id",
  );
  ...
  return readTable_(CONFIG.SHEETS.RESTAURANTS)
    .filter(function (row) {
      ...
      if (
        category &&
        (!categoryRow || String(categoryRow.slug).toLowerCase() !== category)
      )
        return false;
      if (popularOnly && !toBoolean_(row.is_popular)) return false;
      if (search && String(row.name).toLowerCase().indexOf(search) === -1)
        return false;
      return true;
    })
    .map(function (row) {
      ...
      result.delivery_time =
        row.delivery_min_minutes + "-" + row.delivery_max_minutes + " นาที";
      return result;
    });
}

function getRestaurantDetail_(restaurantId) {
  const restaurant = findByIdOrThrow_(
    CONFIG.SHEETS.RESTAURANTS,
    "restaurant_id",
    restaurantId,
  );
...
function listMenu_(restaurantId) {
  const groups = readTable_(CONFIG.SHEETS.CUSTOMIZATION_GROUPS).filter(
    function (row) {
      return toBoolean_(row.is_active);
    },
  );
  const options = readTable_(CONFIG.SHEETS.CUSTOMIZATION_OPTIONS).filter(
    function (row) {
      return toBoolean_(row.is_active);
    },
  );
  ...
  return readTable_(CONFIG.SHEETS.MENU_ITEMS)
    .filter(function (row) {
      return (
        ...
        toBoolean_(row.is_available)
      );
    })
    .map(function (menu) {
      const result = publicRow_(menu);
      result.customization_groups = (
        groupsByMenu[String(menu.menu_item_id)] || []
      )
...
function createCustomer_(body) {
  ...
  if (
    customers.some(function (row) {
      return normalizePhone_(row.phone) === phone;
    })
  ) {
...
function createOrder_(body) {
  ...
  const customer = findByIdOrThrow_(
  ...
  const address = findByIdOrThrow_(
  ...
  const menuById = indexBy_(
    readTable_(CONFIG.SHEETS.MENU_ITEMS),
    "menu_item_id",
  );
  const groupById = indexBy_(
    readTable_(CONFIG.SHEETS.CUSTOMIZATION_GROUPS),
    "group_id",
  );
  const optionById = indexBy_(
    readTable_(CONFIG.SHEETS.CUSTOMIZATION_OPTIONS),
    "option_id",
  );
  ...
  items.forEach(function (requestedItem) {
    const menuId = required_(
      requestedItem.menu_item_id,
      "items[].menu_item_id",
    );
    const menu = menuById[String(menuId)];
    if (
      !menu ||
      String(menu.restaurant_id) !== String(restaurantId) ||
      !toBoolean_(menu.is_available)
    ) {
      throw apiError_("MENU_NOT_AVAILABLE", "เมนูไม่พร้อมขาย: " + menuId, 400);
    }

    const quantity = positiveInteger_(
    ...
    const selectedOptions = selectedOptionIds.map(function (optionId) {
      ...
      if (!option || !toBoolean_(option.is_active)) {
        throw apiError_(
          ...
          "ตัวเลือกไม่พร้อมใช้งาน: " + optionId,
          400,
        );
      }
      ...
      if (
        !group ||
        String(group.menu_item_id) !== String(menuId) ||
        !toBoolean_(group.is_active)
      ) {
        throw apiError_(
          "OPTION_NOT_FOR_MENU",
          "ตัวเลือกไม่ตรงกับเมนู: " + optionId,
          400,
        );
      }
    ...
    readTable_(CONFIG.SHEETS.CUSTOMIZATION_GROUPS)
      ...
      .forEach(function (group) {
        ...
        const min = Number(
          group.min_select || (toBoolean_(group.is_required) ? 1 : 0),
        );
        const max = Number(
          group.max_select ||
            (String(group.selection_type) === "radio" ? 1 : 999),
        );
        if (count < min || count > max) {
          throw apiError_(
            "INVALID_OPTION_COUNT",
            "จำนวนตัวเลือกไม่ถูกต้องในกลุ่ม " + group.title,
            400,
          );
        }
      });
  ...
  const deliveryFee =
    subtotal >= CONFIG.FREE_DELIVERY_MINIMUM ? 0 : CONFIG.DELIVERY_FEE;
  const discount = 0;
  ...
  const paymentStatus =
    paymentMethod === "เงินสดปลายทาง" ? "pending" : "pending";
  ...
  normalizedItems.forEach(function (item) {
    ...
    item.selectedOptions.forEach(function (selected) {
      appendObject_(CONFIG.SHEETS.ORDER_ITEM_OPTIONS, {
        order_item_option_id: nextId_(
          "OIO",
          CONFIG.SHEETS.ORDER_ITEM_OPTIONS,
          "order_item_option_id",
          10,
        ),
        order_item_id: orderItemId,
...
function getOrder_(params) {
  ...
  if (!orderId && !orderNumber) {
    throw apiError_(
      "MISSING_PARAMETER",
      "ต้องระบุ order_id หรือ order_number",
      400,
    );
  }
  ...
  const orderItems = readTable_(CONFIG.SHEETS.ORDER_ITEMS).filter(
    function (row) {
      return String(row.order_id) === String(order.order_id);
    },
  );
  const itemIds = {};
  orderItems.forEach(function (row) {
    itemIds[String(row.order_item_id)] = true;
  });
  const optionsByItem = groupBy_(
    readTable_(CONFIG.SHEETS.ORDER_ITEM_OPTIONS).filter(function (row) {
      return itemIds[String(row.order_item_id)];
    }),
    "order_item_id",
  );
  ...
  result.items = orderItems.map(function (item) {
    ...
    itemResult.options = (optionsByItem[String(item.order_item_id)] || []).map(
      publicRow_,
    );
    return itemResult;
  });
...
function updatePaymentStatus_(body) {
  ...
  const payment = readTable_(CONFIG.SHEETS.PAYMENTS).find(function (row) {
    return String(row.order_id) === String(orderId);
  });
  if (!payment)
    throw apiError_("PAYMENT_NOT_FOUND", "ไม่พบข้อมูลการชำระเงิน", 404);

  const paidAt = status === "paid" ? now_() : "";
  updateById_(CONFIG.SHEETS.PAYMENTS, "payment_id", payment.payment_id, {
    status: status,
    transaction_ref: String(
      body.transaction_ref || payment.transaction_ref || "",
    ),
    paid_at: paidAt,
  });
  updateById_(CONFIG.SHEETS.ORDERS, "order_id", orderId, {
...
function appendStatusHistory_(
  ...
  appendObject_(CONFIG.SHEETS.ORDER_STATUS_HISTORY, {
    history_id: nextId_(
      "HIS",
      CONFIG.SHEETS.ORDER_STATUS_HISTORY,
      "history_id",
      10,
    ),
    order_id: orderId,
...
function validateStatusTransition_(current, next) {
  ...
  if (!allowed[current] || allowed[current].indexOf(next) === -1) {
    throw apiError_(
      ...
      "เปลี่ยนสถานะจาก " + current + " เป็น " + next + " ไม่ได้",
      409,
    );
  }
}
...
function clearDefaultAddresses_(customerId) {
  ...
  values.forEach(function (row, index) {
    if (
      String(row[customerCol]) === String(customerId) &&
      toBoolean_(row[defaultCol])
    ) {
...
function getDb_() {
  ...
  if (!active)
    throw new Error(
      "ไม่พบ Spreadsheet โปรดผูกสคริปต์กับ Google Sheets แล้วรัน setup()",
    );
  return active;
}
...
function updateById_(sheetName, idField, idValue, changes) {
  ...
  if (sheet.getLastRow() < 2)
    throw apiError_("RECORD_NOT_FOUND", "ไม่พบข้อมูล", 404);
  const ids = sheet
    .getRange(2, idColumn + 1, sheet.getLastRow() - 1, 1)
    .getDisplayValues();
  const index = ids.findIndex(function (row) {
    return String(row[0]) === String(idValue);
  });
  if (index === -1)
    throw apiError_("RECORD_NOT_FOUND", "ไม่พบข้อมูล: " + idValue, 404);
  const rowNumber = index + 2;
  Object.keys(changes).forEach(function (field) {
    const column = headers.indexOf(field);
    if (column !== -1)
      sheet.getRange(rowNumber, column + 1).setValue(changes[field]);
  });
}
...
function findByIdOrThrow_(sheetName, field, value) {
  ...
  if (!row)
    throw apiError_("RECORD_NOT_FOUND", "ไม่พบ " + field + ": " + value, 404);
  return row;
}
...
function makeOrderNumber_() {
  return (
    Utilities.formatDate(new Date(), CONFIG.TIME_ZONE, "yyyyMMddHHmmss") +
...
function withScriptLock_(callback) {
  ...
  if (!lock.tryLock(30000))
    throw apiError_("BUSY", "ระบบกำลังประมวลผล กรุณาลองใหม่", 503);
  try {
    return callback();
  } finally {
    lock.releaseLock();
...
function nullableNumber_(value) {
  ...
  if (!Number.isFinite(number))
    throw apiError_("INVALID_NUMBER", "ค่าพิกัดไม่ถูกต้อง", 400);
  return number;
}
